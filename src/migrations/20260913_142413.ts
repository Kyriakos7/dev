...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE UNIQUE INDEX \`menu_sections_slug_idx\` ON \`menu_sections\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE TABLE \`menu_sections_locales\` (
  	\`name\` text NOT NULL,
  	\`description\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`menu_sections\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`menu_sections_locales_locale_parent_id_unique\` ON \`menu_sections_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`categories_notes\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`categories_notes_order_idx\` ON \`categories_notes\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`categories_notes_parent_id_idx\` ON \`categories_notes\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`categories_notes_locales\` (
  	\`text\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`categories_notes\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`categories_notes_locales_locale_parent_id_unique\` ON \`categories_notes_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`categories_menurio_id_idx\` ON \`categories\` (\`menurio_id\`);`)
  await db.run(sql`CREATE INDEX \`categories_section_idx\` ON \`categories\` (\`section_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`menu_items_section_idx\` ON \`menu_items\` (\`section_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`menu_sections_locales\`;`)
  await db.run(sql`DROP TABLE \`categories_notes\`;`)
  await db.run(sql`DROP TABLE \`categories_notes_locales\`;`)
